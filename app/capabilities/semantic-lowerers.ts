...
export function validateSemanticLowererManifest(value: unknown): value is SemanticLowererManifest {
  ...
  const keys = ['schema', 'id', 'version', 'family', 'intent', 'keywords', 'preconditions', 'refusals', 'authority', 'input_schema', 'preservation', 'lowering', 'tldraw_primitives', 'installed_tldraw', 'limits', 'fidelity', 'qualification', 'example_ids', 'speculation'];
  if (Object.keys(manifest).some((key) => !keys.includes(key)) || keys.some((key) => !(key in manifest))) return false;
  const strings = (candidate: unknown) => Array.isArray(candidate) && candidate.length <= 24 && candidate.every((item) => typeof item === 'string' && item.length > 0 && item.length <= 240);
  return manifest.schema === FOGWOOD_SEMANTIC_LOWERER_SCHEMA
    ...
    && strings(manifest.keywords) && strings(manifest.preconditions) && strings(manifest.refusals)
    ...
    && manifest.qualification === 'adapter-fixture-tested'
...
const manifests: SemanticLowererManifest[] = [
  {
    ...
    limits: { operations: 1, name_characters: 80 }, fidelity: 'exact', qualification: 'adapter-fixture-tested', example_ids: ['tldraw-example.configuration.disable-pages'], speculation: 'retrieval-only',
  },
  {
    ...
    limits: { operations: 1, coordinate_magnitude: 100000, maximum_inset: 512 }, fidelity: 'exact', qualification: 'adapter-fixture-tested', example_ids: ['tldraw-example.configuration.camera-options'], speculation: 'retrieval-only',
  },
];
